...
process(Stream) :-
  (exit([Direction|_]) ; Direction = miners), % Если направление не задано, то отправляем miners
  format(atom(Command), '~w~n', [Direction]),
  write(Command),
  write(Stream, Command),
  flush_output(Stream),
  retractall(exit(_)),
  (Direction \= miners). % Проверяем, не была ли отправлена команда miners
...
loop(Stream) :-
  ...
  main :-
      setup_call_cleanup(
          tcp_connect(localhost:5555, Stream, []),  % Setup
          (
              format(atom(Username), '~w~n', ['bot_dan']),
